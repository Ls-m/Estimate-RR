...
class CNNLinearModel(nn.Module):
    def __init__(self, input_size=60*125, 
        ...
        self.model = nn.Sequential(
            nn.Conv1d(in_channels=1, out_channels=4, kernel_size=250, padding=0, stride=2),
            nn.BatchNorm1d(4),
            nn.ReLU(),
            nn.Conv1d(in_channels=4, out_channels=8, kernel_size=125, padding=0, stride=2),
            nn.BatchNorm1d(8),
            nn.ReLU(),
            # nn.Conv1d(in_channels=4, out_channels=2, kernel_size=127, padding=0, stride=2),
            # nn.ReLU(),
            nn.Conv1d(in_channels=8, out_channels=4, kernel_size=125, padding=0, stride=1),
            nn.BatchNorm1d(4),
            nn.ReLU(),
            nn.Flatten(start_dim=1),
            nn.Linear(13016//2, hidden_size),
            nn.ReLU(),
            nn.Dropout(dropout),
