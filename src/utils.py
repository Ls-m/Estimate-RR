...
def compute_freq_features(ppg_segments, fs, n_jobs=-1):  # -1 = all cores
    def process_single(segment):
        features = extract_cwt_features(segment, fs, num_scales=25)
        if features is None:
            print("⚠️ extract_cwt_features returned None")
        return features
    print(f"Number of PPG segments: {len(ppg_segments)}")
    if len(ppg_segments)==0:
        return None
    freq_features = Parallel(n_jobs=n_jobs)(
        delayed(process_single)(segment) for segment in tqdm.tqdm(ppg_segments)
    )
...
def augment_ppg_segment(ppg):
    ...
    This preserves the RR label but changes the Signal-to-Noise Ratio.
    """
    ...
    if aug_type == 'noise':
        # Gaussian White Noise (Sensor static)
        noise = np.random.normal(0, 0.05, len(ppg)) * np.std(ppg)
        ppg_aug = ppg_aug + noise
        
    elif aug_type == 'drift':
        # Baseline Wander (Breathing movement artifact)
        ...
        # Short burst of high freq noise (Muscle movement)
